  ...
  void _initialize(TemplateDetailModel template) {
    ...
    for (var sectionIndex = 0; sectionIndex < template.sections.length; sectionIndex++) {
      ...
      for (final field in section.fields) {
        if (field.isList) {
          _setFieldValue(field.key, _profileListValueForField(field.key));
        } else if (field.isListObject) {
          _setFieldValue(field.key, <Map<String, String>>[]);
        } else {
          final initialValue = _profileValueForField(field.key);
